 ...
 * June 8, 2019 - Added mainMenu JButton.
 * </pre>
 * </p>
 */ 
...
{ 
  ...
  private void printScreen()
  {
    ...
    layout.putConstraint(SpringLayout.NORTH, mainMenu, 85, SpringLayout.NORTH, notInLimit2);
    mainMenu.addActionListener(this);
    add(mainMenu);
  }
  
  /**
   * This method will be invoked when the enter key is pressed while the JTextField is selected or
   * the mainMenu button is pressed. If the JTextField triggers the actionPerformed() method, it will check
   * if the String entered is within the character limit. If it is, the String will be passed into the playerName
   * variable of the DistractionAction class and the panel will be switched to the instructions screen
   * for Quick Quiz. Otherwise, a message telling the user that their name is not within the character limit 
   * will be displayed. If the mainMenu button triggers the actionPerformed() method, game.mainMenu() will be called.
   * 
   * @param e The ActionEvent that was triggered by pressing enter while the JTextField was selected or pressing the mainMenu button.
   */   
