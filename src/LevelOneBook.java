...
/**
 * Change Log
 * May 14, 2019 - Created LevelOneBook
 */
public class LevelOneBook extends Screen implements ActionListener, KeyListener{
  /**
   *  The background image of the panel. Displays a library setting. 
   */   
  private JLabel background = new JLabel(new ImageIcon(getClass().getResource("\\src\\Resources\\Book\\Library.jpg")));
  /**
   *  The book to be displayed. Labeled Distractions 101, and is empty. Will be filled though the use
   *  of JLabels.
   */
  private JLabel book = new JLabel(new ImageIcon(getClass().getResource("\\src\\Resources\\Book\\Book.png")));
  /**
  ...
  /**
   * This method will is used to perform actions when a key is pressed.
   * 
   * @param e The KeyEvent to be processed
   */   
  public void keyPressed(KeyEvent e)
  {
    if (e.getKeyCode() == KeyEvent.VK_RIGHT && currentPage < 6)
    {
      currentPage+= 2;
      nextPressed = true;
      updatePages();
    }
    else if (e.getKeyCode() == KeyEvent.VK_LEFT && currentPage > 2)
    {
      currentPage-=2;
      nextPressed = false;
      updatePages();
  ...
  /**
   * This method is invoked when a key is released. It has no purpose in the LevelOneBook class, 
  ...
  /**
   * This method is invoked when a key is typed. It has no purpose in the LevelOneBook class, but must be overridden
